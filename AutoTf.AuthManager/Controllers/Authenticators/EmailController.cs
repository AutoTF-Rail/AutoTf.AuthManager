...
public class EmailController : AuthentikController
{
    ...
    {
        return await HttpHelper.SendGetString($"/api/v3/authenticators/email/?name={name}&ordering={ordering}&page={page}&page_size={pageSize}&search={search}");
    }
    ...
    public async Task<ActionResult<string>> Put(int id, [FromBody, Required] string name)
    {
        ...
        return await HttpHelper.SendPut($"/api/v3/authenticators/email/{id}/", content, false);
    }
    ...
    public async Task<ActionResult<string>> Patch(int id, [FromBody, Required] string name)
    {
        ...
        return await HttpHelper.SendPatch($"/api/v3/authenticators/email/{id}/", content, false);
    }
    
    [HttpGet("{id:int}/used_by")]
    public async Task<ActionResult<string>> UsedBy(int id)
    {
        return await HttpHelper.SendGetString($"/api/v3/authenticators/email/{id}/", false);
    }
}