...
public class SmsController : AuthentikController
{
    ...
    {
        return await HttpHelper.SendGet($"/api/v3/authenticators/sms/?name={name}&ordering={ordering}&page={page}&page_size={pageSize}&search={search}");
    }

    [HttpGet("{id:int}")]
    public async Task<ActionResult<string>> Get(int id)
    {
        return await HttpHelper.SendGet($"/api/v3/authenticators/sms/{id}/");
    }
    ...
    {
        return await HttpHelper.SendGet($"/api/v3/authenticators/sms/{id}/", false);
    }
}